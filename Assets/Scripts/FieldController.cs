...
public class FieldController : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        reward_shaper = m_ResetParams.GetWithDefault("reward_shaper", 0f);
    ...
    public Vector3 GetRandomSpawnPosBall()
    {
        ...
        float num2 = 0f;
        float num = Random.value; //between 0.0 and 1.0
        if (num < 0.2f) {
          num2 = 13.65f;
        } else if (num > 0.8f) {
          num2 = -13.5f;
      ...
      //ACTIVATE LINE 135 IF VARIED SPAWN DESIRED
       Vector3 position = new Vector3(0f,0f,0f); //normal spawn mantained
       //Vector3 position = new Vector3(0f,0f,num2);
        ...
        var randomSpawnPos = ballSpawnArea.transform.position + position +
    ...
    public void ResetScene() //RESET POS JUGADORS I RESET BALL
    {
        ...
        foreach (var item in AgentsList)
        {
            ...
            //item.Rb.transform.Rotate(0, Random.Range(0, 360), 0, Space.Self);

            item.Rb.velocity = Vector3.zero;
            item.Rb.angularVelocity = Vector3.zero;
        }

        //zona de respawn aleatòria?. + o -14 cada un temps
